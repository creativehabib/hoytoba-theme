...
type PlaybackContextType = {
  isPlaying: boolean;
  currentTrack: AlbumContents | null;
  currentTime: number;
  duration: number;
  togglePlayPause: () => void;
  playTrack: (track: AlbumContents) => void;
  playNextTrack: () => void;
  playPreviousTrack: () => void;
  setCurrentTime: (time: number) => void;
  setDuration: (duration: number) => void;
  setPlaylist: (songs: AlbumContents[]) => void;
...
export function PlaybackProvider({ children }: { children: ReactNode }) {
  ...
  const [currentTrack, setCurrentTrack] = useState<AlbumContents | null>(null);
  const [currentTime, setCurrentTime] = useState(0);
  const [duration, setDuration] = useState(0);
  const [playlist, setPlaylist] = useState<AlbumContents[]>([]);
